...
const Profile = () => {
    ...
    useEffect(() => {
        if (isSuccess) {
            toast.success(data?.message || "Profile updated"); // ✅ Use `data?.message` first
        }
        if (isError) {
            toast.error(error?.message || "Failed to Update the profile"); // ✅ Use `error?.message` first
        }
...
// git config --global user.email "2k22.csds.2213434@gmail.com"
